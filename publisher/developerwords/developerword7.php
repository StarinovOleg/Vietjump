
<div class="container-fluid">	
	<div class="row">	
		<div class="col-lg-3 col-sm-3 col-xs-3">	</div>
			<div class="col-lg-6 col-sm-6 col-xs-6 bg-primary post2">
						<h3>Глаголы как предлоги:</h3>			
						
						
						
						<div class="text-left">						
							<p class="text"> Некоторые глаголы вьетнамского языка так же могут использоваться как предлоги.</p>	 
							
										
						</div>
						
						<h3>cho, đến, khỏi, ở, qua, theo, tới, vào, về</h3>
						
									
						



		<div class="col-lg-3 col-sm-3 col-xs-3    bg-primary ">	</div>		
	
	
			<button id="BackWordPage" class=" btn-lg" > ПРОДОЛЖИТЬ</button>		
	
			</div>
    </div>
</div>

<script>
 BackWordPage.addEventListener("mousedown", function cheking(event) {
	location.href = 'index.php?developerword8=value'; //переадресация на новую тренировку
 });		
document.getElementById('hideBlockIcon1').style.display = 'block'; //блок  меню курса		
	
</script>